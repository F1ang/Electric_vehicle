...
static void MDHandleCmdIn1msLoop(PSTR_MotorFoc tS_pMotorFoc)
{
    ...
    if ((E_FC_RESET_INIT_OBS == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_INIT_OBS != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
    ...
    } else if ((E_FC_RESET_INIT_ACR == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_INIT_ACR != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
    ...
    } else if ((E_FC_RESET_INIT_SVPWM == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_INIT_SVPWM != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
        InitFocVar(tS_pMotorFoc); // 电流环变量初始化
        InitPmFluxOB(t_bObjId);   // 观测器变量初始化
        tS_pMotorFoc->pMdToMa->nuFineStatus = E_FC_RESET_INIT_SVPWM;
    }

    else if ((E_FC_RESET_INIT_IPD == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_INIT_IPD != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
        InitIPD(t_bObjId); /*IPD初始化*/
        tS_pMotorFoc->pMdToMa->nuFineStatus = E_FC_RESET_INIT_IPD;
    }

    else if ((E_FC_IPD_CW_START == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_IPD_CCW_START != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
        tS_pMotorFoc->pMdToMa->nuFineStatus = tS_pMotorFoc->pMdToMa->nuFineStatus;
    }

    else if ((E_FC_RESET_CHARGE_A == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_CHARGE_A != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
        tS_pMotorFoc->m_pHALHandle->PwmCharge(1); // A相预充电
        tS_pMotorFoc->pMdToMa->nuFineStatus = E_FC_RESET_CHARGE_A;
    } else if ((E_FC_RESET_CHARGE_B == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_CHARGE_B != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
        tS_pMotorFoc->m_pHALHandle->PwmCharge(2); // B相预充电
        tS_pMotorFoc->pMdToMa->nuFineStatus = E_FC_RESET_CHARGE_B;
    } else if ((E_FC_RESET_CHARGE_C == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_CHARGE_C != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
        tS_pMotorFoc->m_pHALHandle->PwmCharge(3); // C相预充电
        tS_pMotorFoc->pMdToMa->nuFineStatus = E_FC_RESET_CHARGE_C;
    } else if ((E_FC_RESET_CHARGE_DONE_INIT == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_CHARGE_DONE_INIT != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
        tS_pMotorFoc->m_pHALHandle->InitPwmChargeEnd(); // 预充电结束后，初始化变量
        tS_pMotorFoc->pMdToMa->nuFineStatus = E_FC_RESET_CHARGE_DONE_INIT;
    } else if ((E_FC_RESET_CLR_BREAKIN_FAULT == tS_pMotorFoc->pMaToMd->nuFineCmd) && (E_FC_RESET_CLR_BREAKIN_FAULT != tS_pMotorFoc->pMdToMa->nuFineStatus)) {
    ...
    case E_MOTOR_SYN: {
        ...
        // if(E_PWM_ON == tS_pMotorFoc->ePWMStatusAck)
        {
            ...
            tS_pMotorFoc->m_pStatCurrDQ->nAxisQ = (((s32)t_pStatCurrAB->nBeta * t_pSinCos->hCos) -
                                                   ((s32)t_pStatCurrAB->nAlph * t_pSinCos->hSin)) >>
                                                  15;
            tS_pMotorFoc->m_pStatCurrDQ->nAxisD = (((s32)t_pStatCurrAB->nAlph * t_pSinCos->hCos) +
                                                   ((s32)t_pStatCurrAB->nBeta * t_pSinCos->hSin)) >>
                                                  15;
...
// 根据采样方式设置默认采样状态
    ...
    {
        if ((E_FC_IPD_CW_START == t_pMaToMd->nuFineCmd) || (E_FC_IPD_CCW_START == t_pMaToMd->nuFineCmd)) {
    ...
    case E_MOTOR_IPD: /*IPD初始位置检测*/
    {
        ...
        tS_pMotorFoc->m_pHALHandle->MCPWMx_RegUpdate(tS_pMotorFoc->pMdToHd);
        if ((E_FC_IPD_CW_START == tS_pMotorFoc->pMaToMd->nuFineCmd) || (E_FC_IPD_CCW_START == tS_pMotorFoc->pMaToMd->nuFineCmd)) {
