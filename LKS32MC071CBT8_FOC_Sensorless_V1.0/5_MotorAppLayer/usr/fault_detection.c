...
void FaultCurrentCheck(STR_VectorUVW *pstruCurrent, u8 t_bObj_ID)
{
    ...
    if ((ABS(pstruCurrent->nPhaseU) > pObj->m_pFaultValue->nOverCurrent) || (ABS(pstruCurrent->nPhaseV) > pObj->m_pFaultValue->nOverCurrent) || (ABS(pstruCurrent->nPhaseW) > pObj->m_pFaultValue->nOverCurrent)) {
...
static void FaultEmptyCheck(STR_MACtrProc *pObj)
{
    ...
    if (pDrvCfgPara->mS_FBEmptyStall.bEmptyProtectEnable == 1) {
        if ((ABS(pObj->m_nQCur_Reference) < pObj->m_pFaultValue->nEmptyCurrent) && (ABS(pObj->mMdToMa.wFreq) > pObj->m_pFaultValue->wEmptySpeed)) /*如果Iq小于设定值，转速大于设定值则时间累加，否则时间清0 */
        {
...
static void FaultStallCheck(STR_MACtrProc *pObj)
{
    ...
    if (pDrvCfgPara->mS_FBEmptyStall.bStallProtectEnable == 1) {
        if (pStallFaultTime->nStartDelay < pDrvCfgPara->mS_FBEmptyStall.nStallDelaytime) // 延时* 5ms，避开开环拖动的影响
        {
            ...
            if (t_nCurrent > pFaultValue->wStallCurMax) // 电机电流大于堵转最大电流
            {
                pFaultValue->nStallCurFlag = 1;
            } else {
                if (t_nCurrent < pFaultValue->wStallCurMin) // 电机电流小于堵转消除电流
                {
...
static void FaultStartCheck(STR_MACtrProc *pObj)
{
    ...
    if (pDrvCfgPara->mS_FBEmptyStall.bStartProtectEnable == 1) {
        if (pstruFaultTime->nStartDelay < pDrvCfgPara->mS_FBEmptyStall.nStartFaultDetectTime) // 开环10s内检测，10s后不再检测  5ms*2000
        {
            ...
            if ((pstruIabcMax->nPhaseU > pObj->m_pFaultValue->nStallCurrent) || (pstruIabcMax->nPhaseV > pObj->m_pFaultValue->nStallCurrent) || (pstruIabcMax->nPhaseW > pObj->m_pFaultValue->nStallCurrent)) // 三相电流幅值超过堵转电流设定值则判定为堵转
            {
                pObj->m_pFaults->B.StartError = 1;   // 启动错误标志位置1
                pObj->m_pFaultValue->nStartFlag = 3; // 启动错误标志位为3
            }

            if ((pObj->m_pAppCommData->bOpenRunFlag == 0) && (pstruFaultTime->nStartDelay > pDrvCfgPara->mS_FBEmptyStall.nStartClosedDetectTime)) // 开环600*5ms = 3s后还没切入闭环，则判断为启动失败，具体时间按照实际项目调整
            {
                pObj->m_pFaults->B.StartError = 1;   // 启动错误标志位置1
...
static void FaultPhaseCheck(STR_MACtrProc *pObj)
{
    ...
    if (pDrvCfgPara->mS_FBEmptyStall.bLossPHProtectEnable == 1) {
        ...
        if ((pstruCurrentAmplitude->nPhA < pObj->m_pFaultValue->nPhaseLossCurrent) /* 三相电流幅值小于设定阈值，累积时间小于设定阈值则判断为缺相*/
            || (pstruCurrentAmplitude->nPhB < pObj->m_pFaultValue->nPhaseLossCurrent) || (pstruCurrentAmplitude->nPhC < pObj->m_pFaultValue->nPhaseLossCurrent)) {
...
static void FaultTempCheck(STR_MACtrProc *pObj)
{
    ...
    if (pDrvCfgPara->mS_FBTempNTC.bOvTempProtectEnable == 1) {
        /* 温度大于设定值，则判定为过温故障; */
        /* 温度值小于一定值则判定为NTC开路   */
        if ((pObj->m_nInvTemp > (s16)pDrvCfgPara->mS_FBTempNTC.m_wMaxAllowedWorkTemp) || (pObj->m_nInvTemp < (s16)pDrvCfgPara->mS_FBTempNTC.m_wMinAllowedWorkTemp)) {
...
void FaultRecover(STR_MACtrProc *pObj)
{
    ...
    s16 t_nTmp;
    /****************过欠压恢复****************/
    if ((t_pFaults->B.VoltOverError == 1) || (t_pFaults->B.VoltUnderError == 1)) { // 母线电压在欠压恢复值和过压恢复值之间则判定为正常状态
        if ((pObj->mMdToMa.nVdc >= t_pFaultValue->nUnderVoltageRecover) && (pObj->mMdToMa.nVdc <= t_pFaultValue->nOverVoltageRecover)) {
